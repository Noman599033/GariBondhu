...
def update_lang_file(filepath, new_keys):
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()

    # Find the end of the window.translations.en/bn object (which is just before the closing '};')
    # Since it might have a trailing comma, we can just insert our keys before the last '};'
    ...
    # Check if there is already a trailing comma before insertion
    preceding_text = content[:insertion_point].strip()
    needs_comma = not preceding_text.endswith(',')
    ...
    # Remove last comma from additions
